...
export default function(state = {}, action) {
  switch (action.type) {
    ...
    case FETCH_POST: //if user already visited PostsIndex component we dont want to lose the data;
      return { ...state, [action.payload.data.id]: action.payload.data }  //the [] are necessary like in obj[id]
    case FETCH_POSTS:
      return _.mapKeys(action.payload.data, 'id');  //converting the array of objects into an object with the keys equal to the ids of the objects and the values the objects themselves
    default:
      return state;
  }
}
